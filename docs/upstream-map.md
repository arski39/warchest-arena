...
| `src/server/GameServer.ts`          |    18 | settle hook in `archiveGame()`; wagered start-gate in `toggle_game_start_timer`; `cancelUnfilledWageredMatch()`; refund in `end()`'s not-started branch; `kick_reason.wager_not_full`; `maxGameDuration` from the hoisted constant; `maybeAutoStartFilledWager()`; empty-lobby reap in `phase()` |
...
| `src/core/worker/WorkerClient.ts`   |     4 | `init_error` handling and a timeout message that says what it does not know; `assetOrigin` on the init message                                                                                                                                                                                   |
| `src/core/worker/Worker.worker.ts`  |     3 | `reportInitError()` and the missing catch on the init promise; sets `__ASSET_ORIGIN__` beside `__CDN_BASE__`                                                                                                                                                                                     |
| `src/core/worker/WorkerMessages.ts` |     3 | `init_error` message type; `assetOrigin` on `InitMessage`                                                                                                                                                                                                                                        |
| `src/core/AssetUrls.ts`             |     3 | `getAssetOrigin()` and the `originBase` argument — the only thing making a worker's asset URLs absolute when `CDN_BASE` is empty                                                                                                                                                                 |

**The Marks column drifts.** It is `grep -c '\[ARENA\]'` at the time a row was
last touched, not a live number — the rows above were corrected only where a
change landed on that file. Re-run the grep before trusting one.
