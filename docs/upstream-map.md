...
| ---------------------------------- | ----: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `src/server/GameServer.ts`         |     8 | settle hook in `archiveGame()`; wagered start-gate in `toggle_game_start_timer`; `cancelUnfilledWageredMatch()`; refund in `end()`'s not-started branch; `kick_reason.wager_not_full` |
...
| `src/server/Worker.ts`             |     5 | wagered-join gate, `POST /:id/wager`, wager info on `GET /:id`, listing/wager mutual exclusion                                                                                        |
| `src/client/Api.ts`                |     4 | `fetchLobbyWager`                                                                                                                                                                     |
| `src/core/Schemas.ts`              |     3 | wallet fields on `ClientJoinMessage`                                                                                                                                                  |
...
| `index.html`                       |     2 | ads/analytics removed; canonical/og from `siteOrigin`/`siteName`; `sourceRepoUrl` in BOOTSTRAP_CONFIG                                                                                 |
| `src/server/RenderHtml.ts`         |     2 | those three vars; logos repointed off the removed proprietary PNGs                                                                                                                    |
| `src/client/ClientEnv.ts`          |     2 | `sourceRepoUrl()` with the empty-string fallback                                                                                                                                      |
| `src/server/GameManager.ts`        |     1 | `cancelUnfilledWageredMatch()` in `tick()`                                                                                                                                            |
| `src/server/Master.ts`             |     1 | AGPL source-URL boot warning                                                                                                                                                          |
...
| `src/core/configuration/Config.ts` |     1 | `sourceRepoUrl` on `BOOTSTRAP_CONFIG`                                                                                                                                                 |
